...
class BaggageDetector:
    ...
    def detect_and_segment(self, image: np.ndarray) -> dict:
        ...
        try:
            ...
            results = self.yolo(image, classes=all_classes, conf=Config.BAG_CONFIDENCE, verbose=False)
                ...
                if result.masks is not None and result.boxes is not None:
                    for seg_mask, box, cls, conf in zip(
                        ...
                        if cls == Config.PERSON_CLASS:
                            ...
                            individual_mask = (seg_mask_resized > 0.5).astype(np.uint8) * 255
            ...
            cleaned_baggage_mask = cv2.bitwise_and(baggage_mask, cv2.bitwise_not(person_mask))
